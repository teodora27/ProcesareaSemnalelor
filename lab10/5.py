...
def verifica_stationaritate(ar_coef):
    # construim polinomul caracteristic
    coef = [1.0] + [-c for c in ar_coef]  
    radacini = radacini_polinom(coef[::-1])  
    moduli = np.abs(radacini)
    return np.all(moduli > 1), radacini
...
print("\nModel AR (greedy):") 
stationar_greedy, radacini_greedy = verifica_stationaritate(x_sparse) 
print("E stationar?", stationar_greedy)
...
print("\nModel AR (L1):") 
stationar_l1, radacini_l1 = verifica_stationaritate(x_l1) 
print("E stationar?", stationar_l1)
